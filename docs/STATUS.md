...
### ▶ S-subagents — IN PROGRESS
- [ ] WP-S-subagents — roles-as-config-layer + spawn-depth limits + event-notify mailbox (NOT poll). Codex `core/src/agent/*` + multi_agents_v2; legacy subagents. NEW module `subagents/*` + pub mod line. Network-free tests with a fake spawner.
